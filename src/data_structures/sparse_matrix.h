...
private:
    ...
    size_t _cols;
    size_t _total;
    hashmap<size_t, hashmap_p<size_t, T>> _row_index;
...
//
//template <typename T>
//sparse_matrix<T>::sparse_matrix(const sparse_matrix<T>& other)
//    : _rows(other._rows),
//      _cols(other._cols),
//      _total(other._total),
//      _row_index(),
...
//    }
//
//    for (auto it = _col_index.begin(); it != _col_index.end(); ++it) {
//        delete it->second;
//        it->second = nullptr;
//    }
}
...
void sparse_matrix<T>::set(size_t row, size_t col, const T& value) {
    hashmap_p<size_t, T> row_data = nullptr;
    try {
        row_data = _row_index.at(row);
    } catch (std::out_of_range&) {
        row_data = std::make_shared<hashmap<size_t, T>>();
