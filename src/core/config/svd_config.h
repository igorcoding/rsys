...
namespace rsys {
    ...
    public:
        config(const D<T>* _ratings, size_t _features_count, float regularization = 0.0f, int max_iterations = 0, bool print_results = true);
    ...
    template <typename T, template <class> class D>
    config<svd<T,D>>::config(const D<T>* ratings, size_t features_count, float regularization, int max_iterations, bool print_results)
            : _ratings(ratings),
